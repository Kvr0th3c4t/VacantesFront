...
export class RegistroComponent {
	...
	registrar() {
		...
		this.usuarioService.registro(nuevoUsuario).subscribe({
			next: () => {
				this.loading = false;
				this.mostrarExito("Ahora puedes iniciar sesión.", () => {
					this.router.navigate(["/login"]);
				});
			},
			error: (err) => {
				this.loading = false;
				const mensaje = err.status === 409 ? "El correo ya está registrado." : "Error al registrar. Intenta nuevamente.";
	...
	private mostrarError(mensaje: string, titulo: string = "Oops..."): void {
		Swal.fire({
			icon: "error",
			title: titulo,
			text: mensaje,
